...
async def create_pool(loop, **kw):
    ...
    global  __pool #将pool定义为全局变量

    __pool = await aiomysql.create_pool(
        host=kw.get('host', 'localhost'), #主机ip,默认为本机
...
async def select(sql, args, size=None):
    ...
    async with __pool.acquire() as conn: #从连接池中获取一个连接，使用完后自动释放
        async with conn.cursor(aiomysql.DictCursor) as cur:#创建一个邮编，返回由dict组成的list
            await cur.execute(sql.replace('?','%S'), args or ())#执行sql，mysql的占位符是%s,和python是一样的，为了方便coding，先用sql的占位符?写sql语句,最后执行时再转换过来
            if size:
                rs = await cur.fetchmany(size) #读取size条数据
            else:
                rs = await cur.fetchall() #返回所有数据
